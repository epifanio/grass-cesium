...
class MapFrame(SingleMapFrame):
    ...
    def _addToolbarVDigit(self):
        ...
        #    
        elif self._mgr.GetPane('globe').IsShown():
            self._mgr.GetPane('globe').Hide()
        #
    ...
    def AddGlobe(self):
        ...
        if not self._mgr.GetPane('globe').IsOk():
            self._mgr.GetPane('2d').Hide()
            self.browser = wx.html2.WebView.New(self) 
            self.browser.LoadURL("http://cesiumjs.org/Cesium/Build/Apps/CesiumViewer/index.html")
            self._mgr.AddPane(self.browser,
                              wx.aui.AuiPaneInfo().
                              Name("globe").Caption(_("globe")).
                              ToolbarPane().Top().CentrePane().
                              LeftDockable(False).RightDockable(False).
                              BottomDockable(False).TopDockable(False).
                              CloseButton(False))
        else:
            self._mgr.GetPane('2d').Hide()
            self._mgr.GetPane('globe').Show()
        self._mgr.Update()
